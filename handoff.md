...
- [ ] `RESEND_API_KEY` — user provided a send-only key (verified working via test send from onboarding@resend.dev); NOT yet set on Azure: `om-agent-rg` is NOT under the om@resso.ai Azure login (Sponsorship/Partner subs checked — no om-agent-rg, no container apps named om-agent-service). It's on the gmail-account Azure sub → user must `az login` with gmail, then set RESEND_API_KEY (+ RESEND_FROM; needs a verified domain on resend.com to send from own address, else onboarding@resend.dev only delivers to the Resend account owner)
- [ ] Remaining agent-service env (same gmail Azure login needed): `GOOGLE_CREDENTIALS_JSON`/`GOOGLE_CALENDAR_ID`, `AZURE_OPENAI_EMBEDDING_DEPLOYMENT`, `CHROMA_DIR=/data/.chroma` + volume

...
- [ ] 🔴 `GOOGLE_OAUTH_REFRESH_TOKEN` — STILL PENDING, token expired (7-day Testing-mode expiry). User must: publish OAuth consent screen to Production, then re-auth at localhost:3000/api/google/connect (dev server left running), then set the new token in Vercel + as a secret on om-agent-service. /book prod booking is BROKEN until this happens.

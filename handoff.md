...
- [x] `SMTP_USER`/`SMTP_PASS` ✅ DONE 2026-07-05: fresh Gmail app pw set in Vercel (all 3 envs, sensitive) + .env.local; prod redeployed; /api/contact verified 200 end-to-end (real email sent) → contact form + checkout/ICS emails LIVE
- [x] `RESEND_API_KEY` ✅ DONE 2026-07-05: set as secret on om-agent-service (gmail Azure login, sub "Azure subscription 1"); domain omkumarsolanki.com VERIFIED on Resend; `RESEND_FROM="Omkumar Solanki <hello@omkumarsolanki.com>"` (user-chosen identity — not om@, name is Omkumar not "Om Kumar"); real sends verified. Sender name also unified in bookings-store.ts ICS/SMTP + config.py (commit b1cc28e).
- [x] `AZURE_OPENAI_EMBEDDING_DEPLOYMENT=text-embedding-3-small` ✅ DONE: deployment created on om-agent-aoai-93a1 (Standard, 50 cap); RAG_MODE vectorless→hybrid; consult verified 200 with KB-grounded answer.
- [x] Agent-service image REDEPLOYED (az containerapp up, rev 0000008) — includes the Google OAuth fallback code + new sender defaults. GOOGLE_OAUTH_CLIENT_ID/CLIENT_SECRET set (secret google-oauth-client-secret); GOOGLE_CALENDAR_ID=primary.
- [x] `GOOGLE_OAUTH_REFRESH_TOKEN` ✅ RE-MINTED 2026-07-05 (user re-authed): new token verified (refresh grant OK, calendar scope), set in Vercel ×3 envs (sensitive) + Azure secret google-oauth-refresh-token, prod redeployed. VERIFIED: prod /api/gcal-book POST → success:true + real event link (/book booking LIVE again); agent-service /chat book_appointment intent proposes REAL free slots from the calendar. ⚠️ IF the OAuth consent screen was NOT published to Production, this token dies again ~2026-07-12 — verify /book weekly or confirm the app is published.
- [ ] ⚠️ CHROMA on Azure Files ABANDONED: share chroma-data created + mounted at /data, but Chroma's SQLite DEADLOCKS over SMB (replica wedged, consult 502'd) → CHROMA_DIR env removed (rev 0000008), Chroma is back on ephemeral container disk (KB index auto-rebuilds on restart; episodic memory doesn't survive restarts). Volume + storage acct omagentstor93a1 still exist unused — reuse only with NFS/premium or a different persistence approach.
...
## Log
- **2026-07-05 (hello@ everywhere)** — Public contact swapped to hello@omkumarsolanki.com across the board (`bcc087e`): all frontend display/mailto (home, /book, /engineering, /consulting incl. CONTACT_EMAIL const, ResumeDocument), API error strings, agent prompts/guardrails, KB (agent-service + the PUBLIC COPIES public/knowledge/portfolio.md + data/om-meta.json — which also still had 3 stale cal.com links, purged), config.py owner_email (email signoff + reply-to + calendar attendee now hello@). KEPT on gmail (functional): SMTP_USER (it IS the gmail login) + CONTACT_EMAIL env fallback (contact-form delivery target). Build green, agent-service redeployed (ch9), prod verified: home + /engineering show hello@, health ok.
...
## Activity (auto)
- 2026-07-05 11:32 · main @2df0ac0 · 0 uncommitted · Δ+114 ctx-lines
